...
const getFreguesias = (request, response) => {
    pool.query('SELECT * FROM freguesias ORDER BY id_freguesia', (error, results) => {
        if(error){
            throw error
        }
...
const getFreguesiaById = (request, response) => {
    ...
    pool.query('SELECT * FROM freguesias WHERE id_freguesia = $1', [id], (error, results) => {
        if(error){
            throw error
        }
        response.status(200).json(results.rows)
...
const createFreguesia = (request, response) => {
    ...
    pool.query('INSERT INTO freguesias (nome_freguesia) VALUES ($1)', [nome_freguesia], (error, results) => {
        if(error){
            throw error
        }
...
const deleteFreguesia = (request, response) => {
    ...
    pool.query('DELETE FROM freguesias WHERE id_freguesia = $1', [id], (error, results) => {
        if(error){
            throw error
        }
...
const getUtilizadores = (request, response) => {
    pool.query('SELECT * FROM utilizador ORDER BY id_utilizador', (error, results) => {
        if(error){
            throw error
        }
        response.status(200).json(results.rows)
...
const getUtilizadorById = (request, response) => {
    ...
    pool.query('SELECT * FROM utilizador WHERE id_utilizador = $1', [id], (error, results) => {
        if(error){
            throw error
        }
        response.status(200).json(results.rows);
...
const createUtilizador = (request, response) => {
    const { nome_utilizador, email_utilizador, telemovel_utilizador, password_utilizador, fk_tipo_utilizador } = request.body;

    bcrypt.hash(password_utilizador, saltRounds, function(err, hash) {
        // Store hash in your password DB
        pool.query('INSERT INTO utilizador (nome_utilizador, email_utilizador, telemovel_utilizador, password_utilizador, fk_tipo_utilizador) VALUES ($1,$2,$3,$4,$5)', 
        [nome_utilizador, email_utilizador, telemovel_utilizador, hash, fk_tipo_utilizador], 
        (error, results) => {
            if(error){
                throw error;
            }
...
const updateUtilizador = (request, response) => {
    const id = parseInt(request.params.id)
    const { nome_utilizador, email_utilizador, telemovel_utilizador} = request.body;
...
const deleteUtilizador = (request, response) => {
    ...
    pool.query('DELETE FROM utilizador WHERE id_utilizador = $1', 
    [id], 
    (error, results) => {
        if(error){
            throw error
...
const createOcorrencia = (request, response) =>{
    ...
    const { titulo_ocorrencia, descricao_ocorrencia, data_ocorrencia, latitude_ocorrencia, longitude_ocorrencia, rua_ocorrencia, fk_fotografia, fk_freguesia,fk_distrito,fk_estado, fk_utilizador } = request.body
    ...
    pool.query(
        ...
        [ titulo_ocorrencia, descricao_ocorrencia, data_ocorrencia, latitude_ocorrencia, longitude_ocorrencia, rua_ocorrencia, fk_fotografia, fk_freguesia,fk_distrito,fk_estado, fk_utilizador],
        (error, results) => {
            if(error){
                throw error
            }else
            response.status(200).json(results.rows[0].id_ocorrencia)
        }
    )
}
...
const updateOcorrencia = (request, response) => {
    const id = parseInt(request.params.id)
    const { comentario_ocorrencia, data_ultima_atualizacao_ocorrencia, fk_estado } = request.body
    pool.query(
...
const getFotografias = (request, response) => {
    pool.query('SELECT * FROM fotografias ORDER BY id_fotografia', (error, results) => {
        if(error){
            throw error
        }
        response.status(200).json(results.rows)
...
const getFotografiaById = (request, response) => {
    const id = parseInt(request.params.id);
    pool.query('SELECT * FROM fotografias WHERE id_fotografia = $1', [id], (error, results) => {
        if(error){
            throw error
        }
        response.status(200).json(results.rows)
...
const createFotografia = (request, response) => {
    ...
    pool.query('INSERT INTO fotografias (url_fotografia) VALUES ($1) RETURNING *', [url_fotografia], (error, results) => {
        if(error){
            throw error
        }
